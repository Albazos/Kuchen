...
Nach dem Start zeigt die KuchenApp ein dunkles Hauptfenster mit blauen
Akzentfarben. Die Oberflaeche ist in eine linke Seitenleiste und einen grossen
...
\midrule
\textbf{Sidebar} & Enth\"alt die Hauptaktionen \emph{Send mails}, \emph{Open mail list}, \emph{Import CSV}, \emph{Save} und \emph{Change class}. Ausserdem befinden sich dort die beiden Sortier-Dropdowns. \\
\textbf{Search} & Filtert die Tabelle nach dem eingegebenen Begriff. \\
\textbf{Sorting} & Auswahl der Spalte und der Richtung ueber zwei Dropdown-Felder (\emph{Ascending}/\emph{Descending}). \\
\textbf{Toolbar} & Aktionen direkt ueber der Tabelle: \emph{Delete}, \emph{Import}, \emph{Save}, \emph{+ Add column}, \emph{+ New entry}. \\
...
Datenzeilen, jeweils durch ein Trennzeichen wie Komma oder Semikolon getrennt.
Beispiel:
\begin{center}
\texttt{Name,CakeCount,Hanuta,Waffel,Date} \\
\texttt{Max Mustermann,1,,Apfelmus,01.05.2026} \\
\texttt{Erika Beispiel,2,Ja,,08.05.2026}
\end{center}
\end{infobox}
...
\begin{itemize}[leftmargin=1.5em]
    ...
    \item \textbf{Hinzufuegen:} Klicke auf \textbf{+ New entry}, um eine leere Zeile am Ende der Tabelle einzufuegen.
...
\begin{itemize}[leftmargin=1.5em]
    \item In der linken Seitenleiste im Bereich \textbf{SORTING} eine Spalte sowie
          \emph{Ascending} oder \emph{Descending} auswaehlen -- die Tabelle wird sofort umsortiert.
    \item Im Feld \textbf{Search} einen Begriff eingeben -- die Tabelle zeigt nur noch
          passende Zeilen an. Ist im Sortierfeld eine Spalte ausgewaehlt, wird nur in
          dieser Spalte gesucht; andernfalls wird die gesamte Tabelle durchsucht.
\end{itemize}

\subsection{Neue Spalten hinzufuegen}

\begin{warnbox}[Aktueller Stand]
Der Button \textbf{+ Add column} ist in der Oberflaeche bereits vorhanden, die
Funktion ist aktuell aber noch nicht umgesetzt. Beim Anklicken erscheint nur
eine Informationsmeldung.
\end{warnbox}

\subsection{Aenderungen speichern}

Klicke auf \textbf{Save} in der Sidebar oder in der Toolbar, um die aktuellen
Daten zurueck in die CSV-Datei zu schreiben. Beim Beenden der Anwendung wird
ausserdem gefragt, ob ungespeicherte Aenderungen gesichert werden sollen.
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item Es oeffnet sich ein separater Dialog im gleichen dunklen Design mit
          eigener Sidebar, Search-Feld und Toolbar.
    \item Pflege die Eintraege wie in der Hauptliste (bearbeiten, hinzufuegen,
          loeschen) und schliesse den Dialog ueber \textbf{Close} unten rechts.
\end{enumerate}
\end{stepbox}
...
\begin{warnbox}[Weitere Spalten]
Auch im Maillisten-Dialog gibt es bereits einen Button \textbf{+ Add column}.
Wie im Hauptfenster ist diese Funktion im Moment nur vorbereitet und noch nicht
fertig umgesetzt.
\end{warnbox}
...
\begin{enumerate}[leftmargin=1.5em]
    ...
    \item Klicke auf \textbf{Send mails}.
    \item Bestaetige die Sicherheitsabfrage \emph{"`Mail an X Empfaenger
          senden?"'} mit \textbf{Ja}.
    \item Im folgenden Dialog Server, Benutzername und Passwort eingeben.
    \item Optional den \textbf{Subject}-Text anpassen.
    \item Auf \textbf{Send mails} klicken -- die Mails werden ueber den
...
so seinen Eintrag direkt erkennen. Der Betreff ist im Versanddialog editierbar.
